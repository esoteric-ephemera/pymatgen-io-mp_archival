...
class PoscarArchive(Archiver):
    ...
    #parsed_objects : dict[str,Any] = {"POSCAR": None}
    ...
    def __post_init__(self):
        if isinstance(self.parsed_objects["POSCAR"], Structure):
            ...
            self.parsed_objects = Poscar.from_file(self.parsed_objects["POSCAR"])
    ...
    def to_group(self, group : h5py.Group| zarr.Group, group_key : str = "structure") -> None:
        ...
        group[group_key].attrs["charge"] = 0.
        ...
        group[f"{group_key}/sites"].create_dataset("species", data = [site.species_string for site in structure.sites], **self.compression)
        group[f"{group_key}/sites"].create_dataset("direct_coordinates", data = [site.frac_coords for site in structure.sites], **self.compression)
        if len(structure.site_properties) > 0.:
...
class DosArchive(Archiver):
    ...
    def from_vasprun(cls, vasprun : Vasprun | str | Path, **kwargs):
        ...
        return cls(parsed_objects = {"DOS": vasprun.complete_dos}, **kwargs)
    ...
    def to_group(self, group : h5py.Group, group_key : str = "DOS") -> None:
        ...
        group[group_key].create_dataset("index",
            data = [
                "energies",
...
class BandStructureArchive(Archiver):
    ...
    def to_group(self, group : h5py.Group | zarr.Group, group_key : str = "group") -> None:
        ...
        group[group_key].create_dataset("complete_band_structure", data = bs_data, dtype=self.float_dtype, **self.compression)
        group[group_key].create_dataset("reciprocal_lattice", data=self.bs.lattice_rec.matrix, dtype=self.float_dtype, **self.compression)
        ...
        if (bs_proj := self.bs.projections) is not None:
            ...
            for spin, sr_proj_data in bs_proj.items():
                group[f"{group_key}/projections"].create_dataset(str(spin),data=sr_proj_data, dtype=self.float_dtype, **self.compression)
...
class ElectronicStructureArchive(Archiver):
    ...
    def from_vasprun(cls, vasprun : str | Path | Vasprun = "vasprun.xml", **kwargs):
        ...
        return cls(parsed_objects = {"DOS": vasprun.complete_dos, "BS": vasprun.get_band_structure(efermi="smart")}, **kwargs)
    ...
    def from_task_doc(cls, task_doc : TaskDoc, **kwargs):
        ...
        return cls(parsed_objects = {"DOS" : es_objs["DOS"], "BS" : es_objs["BANDSTRUCTURE"]}, **kwargs)
...
class VolumetricArchive(Archiver):
        ...
        for file_name, obj in self.parsed_objects.items():
            ...
            for aug_key, data_aug_subset in obj.data_aug.items():
                if data_aug_subset is not None:
                    self.parsed_objects[file_name]["augmentation"][aug_key], _ = self.parse_augmentation_data(data_aug_subset)
    ...
    @classmethod
    def from_directory(cls, calc_dir : str | Path, **kwargs) -> None:
        ...
        for file_name in _VASP_VOLUMETRIC_FILES:
            ...
            if file_path.exists():
                metadata["file_paths"][file_name] = file_path
            ...
            if entry["augmentation"] is not None:
                ...
                for aug_key, aug_data in entry["augmentation"].items():
                    ...
                    for aug_idx, aug_set in enumerate(aug_data):
                        group[f"{file_name}/augmentation/{aug_key}"].create_dataset(f"{aug_idx+1}", data = aug_set, dtype=self.float_dtype, **self.compression)
    
    @staticmethod
    def get_vol_data_from_archive(file_name : str | Path, files_to_retrieve = _VASP_VOLUMETRIC_FILES, fmt: str | None = None) -> dict[str, VolumetricData]:
        ...
        if fmt == ArchivalFormat.HDF5:
            group = h5py.File(file_name, "r")
        elif fmt == ArchivalFormat.ZARR:
            group = zarr.open(file_name,"r")
        ...
        for file_name in files_to_retrieve:
            if group.get(file_name):
                ...
                kwargs = {"poscar": poscar, "data": data}
...
class RawArchive(Archiver):
    ...
    def from_directory(cls, calc_dir : str | Path, **kwargs):
        ...
        parsed_objects = {calc_type: {} for calc_type in _RAW_DATA_HIERARCHY}
        for calc_type, file_list in _RAW_DATA_HIERARCHY.items():
            ...
            for file_name in file_list:
                if (file_path := zpath(calc_dir / file_name)).exists():
                    metadata["file_paths"][calc_type][file_name] = file_path
